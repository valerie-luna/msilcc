...
int main() {
  // Tests disabled because the CLR doesn't let you do that the way chibicc can
  // we don't know the position of variables in relation to each other
  ASSERT(3, ({ int x=3; *&x; }));
  ASSERT(3, ({ int x=3; int *y=&x; int **z=&y; **z; }));
  FAKE_ASSERT(5, ({ int x=3; int y=5; *(&x+1); }));
  FAKE_ASSERT(3, ({ int x=3; int y=5; *(&y-1); }));
  ASSERT(5, ({ int x=3; int *y=&x; *y=5; x; }));
  FAKE_ASSERT(7, ({ int x=3; int y=5; *(&x+1)=7; y; }));
  FAKE_ASSERT(7, ({ int x=3; int y=5; *(&y-2+1)=7; x; }));
