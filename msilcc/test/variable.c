...
int main() {
  ...
  // we're not in control of local positions on the CLR
  // so this won't work
  FAKE_ASSERT(7, ({ int x; int y; char z; char *a=&y; char *b=&z; b-a; }));
  FAKE_ASSERT(1, ({ int x; char y; int z; char *a=&y; char *b=&z; b-a; }));
